...
@Component({
  selector: 'app-cood-courrier-entrant',
  templateUrl: 'cood-centrant.component.html',
  styleUrls: ['./cood-centrant.component.css'],
  animations: [
    trigger('flyInOut', [
...
export class CoodCourrierEntrantComponent extends EditableListComponent implements OnInit {
  ...
  onDoCloseOpenFilter() {
    if(this.showClose && this.showOpen) {
      this.dataHelper.query = [
        {or: false, filters:[new Filter('ExpediteurExterneId', this.coordonneeID,  'eq')]},
      ];
    } else if(this.showOpen && !this.showClose) {
      this.dataHelper.query = [
        {or: false, filters:[new Filter('ExpediteurExterneId', this.coordonneeID,  'eq'), new Filter('is_closed', 0, 'eq')]},
      ];
    } else if (this.showClose && !this.showOpen) {
      this.dataHelper.query = [
        {or: false, filters:[new Filter('ExpediteurExterneId', this.coordonneeID,  'eq'), new Filter('is_closed', 1, 'eq')]},
      ];
    } else {
      this.dataHelper.query = [
        {or: false, filters:[new Filter('ExpediteurExterneId', this.coordonneeID,  'eq'), new Filter('id', '', 'eq')]},
      ];
    }
    this.dataHelper.loadData(1);
  }
    ...
    this.route.parent.data.subscribe((data: { coordonnee: ICrCoordonnee }) =>
      {
        ...
        const queryOptions = new QueryOptions(
          [
              {or: false, filters: [
                new Filter('ExpediteurExterneId', data.coordonnee.id,  'eq'),
